...
void write_ilm_asmaa_final_result(Server *final_data, char *path, char *name) {
  ...
  for(unsigned int i=0; i<final_data->size; i++){
    max_size = final_data->words_structs[i].size * (final_data->words_structs[i].size > max_size) + max_size * !(final_data->words_structs[i].size > max_size);
  ...
  for(unsigned int j=0; j<max_size; j++){
	...
	strcpy(result+(result_index), final_data->words_structs[a].words[j * (j < final_data->words_structs[a].size)].string);
	result_index += strlen(final_data->words_structs[a].words[j * (j < final_data->words_structs[a].size)].string);
      }else{
...
Words *ilm_asmaa_comparison_same_word_size(Words *words, Words *assets, unsigned int assets_n){
  ...
  for(unsigned int a=0; a<words->size; a++){
    for(unsigned int b=0; b<assets_n; b++){
      for(unsigned int c=0; c<assets[b].size; c++){
	if(words->words[a].number_of_chars != assets[b].words[c].number_of_chars){
          break;
	}
...
Word *load_arabic_words(char *path, unsigned short number_of_characters, unsigned int *result_size) {
  ...
  *result_size = words_size;
  ...
  //  printf("word = %s, ism_value = %d\n", words.words[i].string, words.words[i].name_value);
...
void calculate_values(Words *words) {
  ...
  for (unsigned int i = 0; i < words->size; i++) { 
    for (unsigned int j = 0; j < words->words[i].number_of_chars; j++) {
      ...
      // الف
      words->words[i].name_value += الف * ( words->words[i].string[j*2] == "ا"[0] && words->words[i].string[j*2+1] == "ا"[1] );
      words->words[i].name_value += الف * ( words->words[i].string[j*2] == "أ"[0] && words->words[i].string[j*2+1] == "أ"[1] );
      words->words[i].name_value += الف * ( words->words[i].string[j*2] == "ء"[0] && words->words[i].string[j*2+1] == "ء"[1] );
      words->words[i].name_value += الف * ( words->words[i].string[j*2] == "ؤ"[0] && words->words[i].string[j*2+1] == "ؤ"[1] );
      words->words[i].name_value += الف * ( words->words[i].string[j*2] == "ئ"[0] && words->words[i].string[j*2+1] == "ئ"[1] );
      words->words[i].name_value += الف * ( words->words[i].string[j*2] == "آ"[0] && words->words[i].string[j*2+1] == "آ"[1] );
      words->words[i].name_value += الف * ( words->words[i].string[j*2] == "ى"[0] && words->words[i].string[j*2+1] == "ى"[1] );
      words->words[i].name_value += ( الف + لام) * ( words->words[i].string[j*2] == "ﻻ"[0] && words->words[i].string[j*2+1] == "ﻻ"[1] );
      words->words[i].name_value += ( الف + لام) * ( words->words[i].string[j*2] == "ﻵ"[0] && words->words[i].string[j*2+1] == "ﻵ"[1] );
      words->words[i].name_value += ( الف + لام) * ( words->words[i].string[j*2] == "ﻷ"[0] && words->words[i].string[j*2+1] == "ﻷ"[1] );
      // الف

      // باء
      words->words[i].name_value += باء * ( words->words[i].string[j*2] == "ب"[0] && words->words[i].string[j*2+1] == "ب"[1] );
      // باء

      // جيم
      words->words[i].name_value += جيم * ( words->words[i].string[j*2] == "ج"[0] && words->words[i].string[j*2+1] == "ج"[1] );
      // جيم
      
      // دال
      words->words[i].name_value += دال * ( words->words[i].string[j*2] == "د"[0] && words->words[i].string[j*2+1] == "د"[1] );
      // دال
      
      // هاء
      words->words[i].name_value += هاء * ( words->words[i].string[j*2] == "ه"[0] && words->words[i].string[j*2+1] == "ه"[1] );
      // هاء
      
      // واو
      words->words[i].name_value += واو * ( words->words[i].string[j*2] == "و"[0] && words->words[i].string[j*2+1] == "و"[1] );
      // واو
      
      // زاي
      words->words[i].name_value += زاي * ( words->words[i].string[j*2] == "ز"[0] && words->words[i].string[j*2+1] == "ز"[1] );
      // زاي
      
      // حاء
      words->words[i].name_value += حاء * ( words->words[i].string[j*2] == "ح"[0] && words->words[i].string[j*2+1] == "ح"[1] );
      // حاء
      
      // طاء
      words->words[i].name_value += طاء * ( words->words[i].string[j*2] == "ط"[0] && words->words[i].string[j*2+1] == "ط"[1] );
      // طاء
      
      // ياء
      words->words[i].name_value += ياء * ( words->words[i].string[j*2] == "ي"[0] && words->words[i].string[j*2+1] == "ي"[1] );
      // ياء
      
      // كاف
      words->words[i].name_value += كاف * ( words->words[i].string[j*2] == "ك"[0] && words->words[i].string[j*2+1] == "ك"[1] );
      // كاف
      
      // لام
      words->words[i].name_value += لام * ( words->words[i].string[j*2] == "ل"[0] && words->words[i].string[j*2+1] == "ل"[1] );
      // لام

      // ميم
      words->words[i].name_value += ميم * ( words->words[i].string[j*2] == "م"[0] && words->words[i].string[j*2+1] == "م"[1] );
      // ميم
      
      // نون
      words->words[i].name_value += نون * ( words->words[i].string[j*2] == "ن"[0] && words->words[i].string[j*2+1] == "ن"[1] );
      // نون

      // سين
      words->words[i].name_value += سين * ( words->words[i].string[j*2] == "س"[0] && words->words[i].string[j*2+1] == "س"[1] );
      // سين

      // عين
      words->words[i].name_value += عين * ( words->words[i].string[j*2] == "ع"[0] && words->words[i].string[j*2+1] == "ع"[1] );
      // عين

      // فاء
      words->words[i].name_value += فاء * ( words->words[i].string[j*2] == "ف"[0] && words->words[i].string[j*2+1] == "ف"[1] );
      // فاء

      // صاد
      words->words[i].name_value += صاد * ( words->words[i].string[j*2] == "ص"[0] && words->words[i].string[j*2+1] == "ص"[1] );
      // صاد

      // قاف
      words->words[i].name_value += قاف * ( words->words[i].string[j*2] == "ق"[0] && words->words[i].string[j*2+1] == "ق"[1] );
      // قاف

      // راء
      words->words[i].name_value += راء * ( words->words[i].string[j*2] == "ر"[0] && words->words[i].string[j*2+1] == "ر"[1] );
      // راء

      // شين
      words->words[i].name_value += شين * ( words->words[i].string[j*2] == "ش"[0] && words->words[i].string[j*2+1] == "ش"[1] );
      // شين

      // تاء
      words->words[i].name_value += تاء * ( words->words[i].string[j*2] == "ت"[0] && words->words[i].string[j*2+1] == "ت"[1] );
      // تاء

      // ثاء
      words->words[i].name_value += ثاء * ( words->words[i].string[j*2] == "ث"[0] && words->words[i].string[j*2+1] == "ث"[1] );
      // ثاء

      // خاء
      words->words[i].name_value += خاء * ( words->words[i].string[j*2] == "خ"[0] && words->words[i].string[j*2+1] == "خ"[1] );
      // خاء

      // ذال
      words->words[i].name_value += ذال * ( words->words[i].string[j*2] == "ذ"[0] && words->words[i].string[j*2+1] == "ذ"[1] );
      // ذال

      // ضاد
      words->words[i].name_value += ضاد * ( words->words[i].string[j*2] == "ض"[0] && words->words[i].string[j*2+1] == "ض"[1] );
      // ضاد

      // ظاء
      words->words[i].name_value += ظاء * ( words->words[i].string[j*2] == "ظ"[0] && words->words[i].string[j*2+1] == "ظ"[1] );
      // ظاء

      // غين
      words->words[i].name_value += غين * ( words->words[i].string[j*2] == "غ"[0] && words->words[i].string[j*2+1] == "غ"[1] );
      // غين
